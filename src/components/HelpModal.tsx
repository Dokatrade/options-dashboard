    ...
    <div style={{position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60}}>
      <div style={{background: 'var(--card)', color: 'var(--fg)', border: '1px solid var(--border)', borderRadius: 12, width: 800, maxWidth: '95%', maxHeight: '90%', overflow: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,.35)'}}>
        ...
        <div style={{padding: 16, lineHeight: 1.4}}>
          ...
          <p><strong>Окно IF (условия)</strong></p>
          <ul>
            <li>Открывается из строки позиции кнопкой <strong>IF</strong>. Каждое правило хранится локально и привязано к конкретной позиции.</li>
            <li>Вверху расположены <strong>шаблоны</strong>: кнопки с ⭐ добавляют сохранённые условия; режим <em>Manage</em> позволяет удалять по одному, выделять несколько и чистить всё.</li>
            <li>Под шаблонами отображается <strong>сводка условий</strong>. Там же можно удалить условие крестиком — это синхронно удалит его в редакторе. Стрелка `→` показывает текущее значение левой части (зелёная, если условие выполняется; серая — если нет).</li>
            <li>Ниже перечислены <strong>блоки</strong> (chains). Блок может работать по всей позиции (<em>Position block</em>) или по конкретной ноге (<em>Leg block</em>). Для leg‑блоков есть выпадающий список: <em>Any leg</em> или конкретный символ.</li>
            <li>
              Внутри блока задайте одно или несколько условий. Каждое состоит из <em>Left side</em> (Operand), <em>Comparator</em> (<code>&gt;</code>, <code>&gt;=</code>, <code>&lt;</code>, <code>&lt;=</code>, <code>=</code>) и <em>Right side</em>.
              <div>Доступные типы операндов:</div>
              <ul style={{marginTop:4, marginBottom:4}}>
                <li><strong>Number</strong> — ввод произвольного значения.</li>
                <li><strong>Position</strong> — выберите метрику позиции (spot, net mid, PnL, Δ, Θ и др.).</li>
                <li><strong>Leg</strong> — метрика отдельной ноги (mid, bid/ask, PnL ноги, Δ, IV%, OI, Δσ). В leg‑блоке можно переключаться между «This leg» и конкретным символом; в position‑блоке символ выбирается явно.</li>
              </ul>
            </li>
            <li>Кнопка <strong>+ Operand</strong> добавляет арифметический компонент (сложение, вычитание, умножение или деление) к базовому операнду. Деление трактуется как коэффициент и отображается без знака валюты.</li>
            <li>Статусная точка рядом с условием подсвечивается зелёным, если текущее состояние рынка удовлетворяет правилу. Кнопка <em>Save template</em> сохраняет конкретное условие вместе с текущим выбором блока (в т.ч. Any leg/символ).</li>
            <li>Добавляйте новые условия кнопкой <strong>+ Add condition</strong>; между условиями и блоками выбирайте конъюнкцию (<em>AND</em> / <em>OR</em>).</li>
            <li><strong>Save</strong> сохраняет правило (в `localStorage/if-rules-v3`), <strong>Cancel</strong> закрывает окно без изменений. Удаление последнего условия в блоке автоматически убирает блок.</li>
            <li>Шаблоны (`if-templates-v1`) общие для всех позиций. При сохранении leg‑шаблона фиксируется выбранный символ или явное значение “Any leg”.</li>
          </ul>
